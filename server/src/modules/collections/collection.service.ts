...
function isGeneratorRole(
  role: string
): role is "GENERATOR_SMALL" | "GENERATOR_LARGE" {
  return (
    role === "GENERATOR_SMALL" ||
    role === "GENERATOR_LARGE"
  );
}
...
export class CollectionService {
  ...
  > {
    ...
    const normalized =
      materialsInput.map(
        (
          ...
          if (!name) {
            throw new CollectionDomainError(
              `Informe o nome do material na posição ${
                index + 1
              }.`,
              {
                statusCode: 400,
    ...
    for (
      const material of normalized
    ) {
      const key =
        material.wasteTypeId
          ? `catalog:${material.wasteTypeId}:${material.unit}`
          : `proposal:${material.name
              .trim()
              .toLocaleLowerCase(
                "pt-BR"
              )}:${material.unit}`;
        ...
        await transaction.wasteCatalogSuggestion.create(
          {
            data: {
              ...
              origin:
                WasteCatalogSuggestionOrigin.COLLECTION_FOUND,
    ...
    if (
      ...
      throw new CollectionDomainError(
        "Apenas cooperativas podem delegar coletas.",
        {
          statusCode: 403,
          code:
            "ONLY_COOPERATIVE_CAN_CREATE_COLLECTION",
        }
      );
    }
    ...
    return prisma.$transaction(
      async (
        ...
        if (
          ...
          throw new CollectionDomainError(
            "Este agendamento já possui uma coleta vinculada.",
            {
              statusCode: 409,
              code:
                "SCHEDULE_ALREADY_HAS_COLLECTION",
            }
          );
        }
        ...
        if (
          ...
          if (!driver) {
            throw new CollectionDomainError(
              "Motorista não encontrado ou indisponível.",
              {
                statusCode: 404,
                code:
                  "DRIVER_UNAVAILABLE",
        ...
        if (
          ...
          if (!route) {
            throw new CollectionDomainError(
              "Rota não encontrada ou indisponível.",
              {
                statusCode: 404,
                code:
                  "ROUTE_UNAVAILABLE",
  ...
  async start(
    ...
    if (
      ...
      throw new CollectionDomainError(
        "Apenas o catador responsável pode iniciar a coleta.",
        {
          statusCode: 403,
          code:
            "ONLY_COLLECTOR_CAN_START",
        }
      );
    }
    ...
    if (
      ...
      throw new CollectionDomainError(
        "Apenas o catador responsável pode concluir a etapa de campo.",
        {
          statusCode: 403,
          code:
            "ONLY_COLLECTOR_CAN_COMPLETE_FIELD",
        }
      );
    }
    ...
    return prisma.$transaction(
      async (
        ...
        if (
          ...
          throw new CollectionDomainError(
            "Os materiais desta coleta já foram registrados.",
            {
              statusCode: 409,
              code:
                "COLLECTION_MATERIALS_ALREADY_CREATED",
            }
          );
        }
          ...
          await transaction.collection.update(
            {
              ...
              data: {
                ...
                fieldCompletedByUserId:
                  authUserId,

                totalWeightKg,

                materials:
                  normalizedMaterials as unknown as Prisma.InputJsonValue,
  ...
  private async cooperativeTransition(
    ...
    if (
      ...
      throw new CollectionDomainError(
        "Apenas a cooperativa pode executar esta operação.",
        {
          statusCode: 403,
          code:
            "ONLY_COOPERATIVE_CAN_TRANSITION",
        }
      );
    }
  ...
  async startSorting(
    ...
    return this.cooperativeTransition(
      ...
      CollectionStatus.SORTING,
      {
        sortingStartedAt:
          normalizeDate(
            data.sortingStartedAt
          ),

        sortingStartedByUserId:
          authUserId,
    ...
    return prisma.$transaction(
      async (
        ...
        if (
          ...
          throw new CollectionDomainError(
            "A coleta não possui entradas de resíduos.",
            {
              statusCode: 409,
              code:
                "COLLECTION_WITHOUT_WASTE_ENTRIES",
            }
          );
        }
        ...
        if (
          ...
          throw new CollectionDomainError(
            "Ainda existem resíduos com saldo pendente de destinação.",
            {
              statusCode: 409,
              code:
                "COLLECTION_HAS_PENDING_DESTINATIONS",
  ...
  async cancel(
    ...
    if (
      ...
      throw new CollectionDomainError(
        "Apenas a cooperativa pode cancelar uma coleta.",
        {
          statusCode: 403,
          code:
            "ONLY_COOPERATIVE_CAN_CANCEL",
        }
      );
    }
    ...
    return prisma.$transaction(
      async (
        ...
        if (
          ...
          throw new CollectionDomainError(
            "Não é possível cancelar uma coleta já encerrada.",
            {
              statusCode: 409,
              code:
      ...
      case CollectionStatus.CANCELLED:
        return this.cancel(
          ...
          collectionId,
          {
            cancellationReason:
              data.cancellationReason ||
              "Cancelamento solicitado pela rota legada.",
          }
        );

      case CollectionStatus.PENDING:
        throw new CollectionDomainError(
          "Não é permitido retornar uma coleta para PENDING.",
          {
            statusCode: 409,
            code:
              "COLLECTION_CANNOT_RETURN_TO_PENDING",
          }
        );

      default:
        throw new CollectionDomainError(
          "Status de coleta não suportado.",
          {
            statusCode: 400,
            code:
              "UNSUPPORTED_COLLECTION_STATUS",
          }
        );
    }
  }
